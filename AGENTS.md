...
A Nix-managed Python 3.13 project. A data-first knowledge base backed by
SQLite.

## Project structure

```
flake.nix              # Nix flake: dev shell (deps, PYTHONPATH, tools)
pyproject.toml         # Ruff, pytest config (workspace-wide)
justfile               # check / test / lint targets
sqlc.yaml              # sqlc config for SQLite
.coveragerc            # coverage omit rules
conftest.py            # root pytest conftest

database/              # Database connection and generated code
  connection.py        # connect() and migrate() functions
  schema.sql           # Auto-generated from migrations (do not hand-edit)
  __codegen__/         # Auto-generated by sqlc (gitignored)
...
tools/                 # Developer tooling (not shipped)
  codegen_db/          # Orchestrator: runs migrations + sqlc generate
  codegen_db_plugin/   # sqlc codegen plugin: generates Python from SQLite queries
```
...
Use the context manager `database.connection.connect(db_path)` to get a
...
package name, e.g. `database/connection.py`.
...
- The `t.db_path` gives you a `Path` to a session-scoped migrated test
  database. Use `database.connection.connect(t.db_path)` to get a connection.
